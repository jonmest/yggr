...
use jotun_core::{Incoming, LogEntry, LogIndex, Message};
...
fn single_node_cfg(threshold: u64) -> Config {
    ...
    cfg.bootstrap = Bootstrap::NewCluster { members: vec![nid(1)] };
...
async fn wait_for_leader<S: StateMachine>(node: &Node<S>) {
    ...
    loop {
        ...
        if matches!(s.role, jotun::Role::Leader) {
            return;
        }
        assert!(
